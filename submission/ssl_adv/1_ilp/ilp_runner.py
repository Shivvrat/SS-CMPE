...
from ilp_solver_class_one_vs_all import create_solver
...
def run_ilp(debug, test_batch_size, nn_as_dict_of_np_array,  args, loader, date, save_name, dataset="test"):
    """
    ...
    for batch_idx, data in enumerate(tqdm(loader)):
        ...
        for index, each_prediction in enumerate(data):
            inputs[index] = (
                f"ILP model_{index}", nn_as_dict_of_np_array, data[index], target[index], debug, args,
                index)
    ...
    if not debug:
        output_directory = f"ilp_outputs/{save_name}/"
        create_directory(output_directory)
        np.savez(f"{output_directory}/{dataset}_outputs.npz", updated=updated, initial=initial,
                 missed_examples=np.array(missed_examples))
    else:
        output_directory = f"debug/outputs/{date}/"
        create_directory(output_directory)
        np.savez(f"{output_directory}/{dataset}_outputs.npz", updated=updated, initial=initial,
                 missed_examples=np.array(missed_examples))
        logger.info(f"Initial Inputs {initial}")
        logger.info(f"Updated Inputs {updated}")
        updated_values = np.sum(updated != initial)
        logger.info(f"We updated {updated_values / len(initial)} pixels on average")
