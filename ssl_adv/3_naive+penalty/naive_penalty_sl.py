...
def get_total_loss(args, output, target, loss_from_g, f_y, rho=10):
    # Feasible value 
    satisfied_loss = torch.mean(f_y(output, target), dim=1)
...
def train(args, model, n_theta, device, train_loader, optimizer, loss_ae, loss_g, epoch):
    model.train()
    f_y = loss_ae
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss_from_g, outputs_of_g = loss_from_g_alpha(output, loss_g, n_theta, args, device)
...
def train_ae(args, model=None):
    ...
    if not debug and model is None:
        wandb.init(project=f"{select_class}_{class_label}")
    ...
    for epoch in range(1, args.ae_epochs + 1):
        ...
        train(args, autoencoder, n_theta, device, train_loader_for_autoencoder, optimizer, loss_ae, loss_g, epoch)
