...
def get_total_loss(args, data, output, loss_from_g, alpha_val, f_y, current_f_value, unit_g = False):
    ...
    if not args.not_class:
        ...
        # not class
        non_satisfied_loss = -(loss_from_g - 0.5)
    non_satisfied_loss = torch.squeeze(non_satisfied_loss)
    non_satisfied_loss += satisfied_loss
    non_satisfied_loss *= alpha_val
    loss_from_g_relu = torch.nn.functional.relu(non_satisfied_loss.clone())
    if args.penalty:
        penalty = torch.pow(loss_from_g_relu.clone(), 2) * (10/2)
...
def train(args, model, n_theta, device, train_dataset, optimizer, loss_ae, loss_g, epoch, train_kwargs):
    model.train()
    n_theta.eval()
    # using squared loss for f_y right now
    f_y = loss_ae
    ...
    for batch_idx, (batch_data) in enumerate(train_dataloader):
        ...
        if not args.no_entropy_loss:
            entropy_loss = entropy_loss_function(output)
...
def train_ae(args, model=None):
    ...
    if not debug and model is None:
        wandb.init(project=f"ssl_no_lag_{args.numerator_operation}_{select_class}_{class_label}")
        wandb.config.update(args)
    # Define the output directory
    if not args.not_class:
        name = f"{args.class_index}/{args.numerator_operation}_{args.class_index}"
    else:
        name = f"{args.class_index}/not_{args.numerator_operation}_{args.class_index}"
    if args.binarize:
