...
def loss_from_g_alpha(X, y, func, args, device):
...
def get_total_loss(args, output, target, loss_from_g, f_y, rho=10):
    # Feasible value 
    satisfied_loss = torch.sum(f_y(output, target), dim=1)
    loss_from_g = torch.squeeze(loss_from_g)
    # Multiply by rho
    non_satisfied_loss = torch.pow(loss_from_g.clone(), 2) * rho
    non_satisfied_loss +=  satisfied_loss
    return satisfied_loss, non_satisfied_loss 


def train(args, model, device, train_loader, optimizer, loss_ae, func_g, epoch):
    model.train()
    f_y = loss_ae
    train_loss = 0
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss_from_g = loss_from_g_alpha(data, output, func_g, args, device)
        satisfied_loss, non_satisfied_loss = get_total_loss(args, output, target, loss_from_g, f_y)
        val_g_y = loss_from_g.clone().detach()
...
def train_ae(args):
    ...
    if not debug:
        wandb.init(project=f"sl_PENALTY_{name}")
    ...
    if args.loss == "MSE":
        ...
        loss_ae = nn.L1Loss(reduction='none')
    else:
    ...
    for epoch in range(1, args.ae_epochs + 1):
        train(args, model, device, train_loader_for_model, optimizer, loss_ae, func_g, epoch)
        _, _  = validate(args, model, device, train_loader_for_model, loss_ae,  best_loss, counter)# if counter >= patience:
        best_loss, counter  = validate(args, model, device, test_loader_for_model, loss_ae, best_loss, counter)
        # if counter >= patience:
        #     print(
        #         f"Validation loss hasn't improved for {patience} epochs, stopping training..."
        #     )
        #     break
    all_inputs, all_outputs = generate_output_images(model, test_loader_for_model, device)
